...
export const openApiSpec = {
  ...
  paths: {
    ...
    '/manage/projects/{id}/positions/{positionId}': { put: { tags: ['Manage'], summary: 'Guarda una declaración en el borrador con control de concurrencia; requiere item y previous (null al crear)', security: [{ sessionCookie: [], csrf: [] }], responses: { '200': ok('Declaración guardada'), '409': ok('La declaración fue modificada en otra sesión'), ...errorResponses } } },
    ...
    '/manage/projects/{id}/publication-review': { get: { tags: ['Manage'], summary: 'Compara cada campo del borrador con la última revisión publicada y devuelve token de concurrencia para publicar', security: [{ sessionCookie: [] }], responses: { '200': ok('Diferencias por campo y sección, token y revisión base'), ...errorResponses } } },
