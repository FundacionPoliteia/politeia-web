import { sanitizeTags } from './taxonomy';

const API_BASE = process.env.BLOG_API_BASE_URL || '';

function normalizarPost(p) {
  return {
    id: p.id,
    slug: p.slug,
    titulo: p.title || '',
    extracto: p.excerpt || '',
    contenido: p.contentHtml || '',
    fecha: p.publishedAt || p.createdAt || p.updatedAt,
    imagen: p.coverImage || null,
    imagenCard: p.coverImageThumbnail || p.coverImage || null,
    mostrarPortada: p.showCoverInPost !== false,
    autor: p.authorName || '',
    cierreAutor: p.authorNote || '',
    mostrarCierreAutor: p.showAuthorNote === true,
    categoria: p.category || '',
    tags: sanitizeTags(p.tags),
    references: normalizeReferences(p.references),
  };
}

async function fetchApi(path, { noStore = false } = {}) {
  if (!API_BASE) return null;

  try {
    const res = await fetch(`${API_BASE}${path}`, noStore
      ? { cache: 'no-store' }
      : { next: { revalidate: 300 } });
    if (!res.ok) return null;
    return res.json();
  } catch (e) {
    return null;
  }
}

export async function getPosts(cantidad = 12) {
  const data = await fetchApi(`/v1/posts?status=published&limit=${cantidad}`);
  return (data?.items || []).map(normalizarPost);
}

export async function getPost(slug) {
  const data = await fetchApi(`/v1/posts/${encodeURIComponent(slug)}`);
  if (!data?.item) return null;
  const post = normalizarPost(data.item);
  post.autorPerfil = post.autor ? await getPublicAuthorProfile(post.autor) : null;
  return post;
}

export async function getPublicAuthorProfile(autor) {
  const slug = slugifyAuthor(autor);
  if (!slug) return null;
  const data = await fetchApi(`/v1/profile/public/${encodeURIComponent(slug)}`, { noStore: true });
  return data?.item ? normalizarAuthorProfile(data.item) : null;
}

export async function getPublicAuthorProfiles(limit = 12) {
  const data = await fetchApi(`/v1/profile/public?limit=${limit}`, { noStore: true });
  return (data?.items || []).map(normalizarAuthorProfile);
}

export async function getAllSlugs() {
  const data = await fetchApi('/v1/posts?status=published&limit=50');
  return (data?.items || []).map((p) => p.slug).filter(Boolean);
}

export function etiquetasPost(post, fallback = 'Nota') {
  const tags = sanitizeTags(post?.tags);
  return tags.length ? tags : [fallback];
}

export function hrefAutorBlog(autor) {
  return `/blog?autor=${encodeURIComponent(autor || '')}`;
}

function normalizarAuthorProfile(item = {}) {
  return {
    fullName: item.fullName || '',
    authorSlug: item.authorSlug || '',
    description: item.description || '',
    focusArea: item.focusArea || '',
    closingPhrase: item.closingPhrase || '',
    photoUrl: item.photoUrl || '',
    postCount: Number(item.postCount) || 0,
    latestPostTitle: item.latestPostTitle || '',
    latestPostSlug: item.latestPostSlug || '',
    latestPostDate: item.latestPostDate || '',
    categories: Array.isArray(item.categories) ? item.categories.filter(Boolean) : [],
  };
}

function slugifyAuthor(value = '') {
  return String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 90);
}

function normalizeReferences(value) {
  if (!Array.isArray(value)) return [];
  return value
    .map((reference) => ({
      text: typeof reference?.text === 'string' ? reference.text.trim() : '',
      url: typeof reference?.url === 'string' ? reference.url.trim() : '',
    }))
    .filter((reference) => reference.text);
}

export function formatearFecha(iso) {
  try {
    return new Date(iso).toLocaleDateString('es-AR', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    });
  } catch (e) {
    return '';
  }
}
