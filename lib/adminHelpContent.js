...
export const ADMIN_HELP_TOPICS = Object.freeze([
  ...
  helpTopic('blogs-metadata', 'blogs', 'Datos principales', 'Titulo, autor, categoria y extracto definen como se presenta la nota.', {
    ...
    details: 'La categoria agrupa notas en una seccion del blog. Los tags describen temas mas especificos. Si el extracto queda vacio, el sistema genera uno a partir del contenido.',
  ...
  helpTopic('blogs-cover', 'blogs', 'Portada de la nota', 'Podes usar una URL o subir una imagen y decidir si aparece dentro del articulo.', {
    target: 'blog-cover',
    details: 'La portada se usa en cards, enlaces compartidos y avisos por correo. Espera a que termine la carga antes de guardar o cambiar de imagen.',
  ...
  helpTopic('blogs-editor', 'blogs', 'Editor de contenido', 'Da formato al articulo, agrega enlaces, imagenes y tablas desde la barra superior.', {
    target: 'blog-editor',
    details: 'El contenido se guarda junto con la nota. Las marcas de comentarios solo aparecen en el panel de revision y nunca se publican en el blog.',
  ...
  helpTopic('blogs-advanced', 'blogs', 'Opciones avanzadas', 'El slug forma la direccion publica y normalmente no necesita cambios manuales.', {
    target: 'blog-advanced',
    roles: ['reviewer', 'admin'],
    details: 'Cambia el slug solo antes de compartir la nota o cuando exista una razon editorial clara. Modificarlo puede invalidar enlaces que ya circulan.',
  }),
  ...
  helpTopic('access-email', 'access', 'Avisos por email', 'Cada usuario elige que eventos internos tambien quiere recibir por correo.', {
    target: 'access-email',
    roles: ['admin'],
    details: 'Las notificaciones dentro del panel siguen activas. Desactivar esta opcion solo evita el correo para los eventos elegidos.',
  }),
...
export function getAdminHelpTopics(area, roles = []) {
  ...
  return ADMIN_HELP_TOPICS.filter((topic) => topic.area === area && (!topic.roles.length || topic.roles.some((role) => cleanRoles.includes(role))));
}
